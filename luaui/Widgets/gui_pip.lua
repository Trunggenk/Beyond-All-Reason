...
local function DrawProjectile(pID)
	...
	if pDefID then
		...
		if cache.weaponIsFlame[pDefID] then
			...
			if gl4Prim.enabled then
				GL4AddCircle(px + offsetX, pz - offsetZ, particleSize, alpha, mMin(1, r * 1.2), mMin(1, g * 1.2), mMin(1, b * 1.2), r, g, b, 0.55 * alpha, 0)
			end
	...
	elseif pDefID and cache.weaponIsMissile[pDefID] then
		...
		GL4AddQuad(px + sinA * bodyFwd, pz + cosA * bodyFwd, width, height * 0.5, angle, bodyR, bodyG, bodyB, color[4])

		-- Nose cone: narrow tapered quad at the front tip
		local noseFwd = height * 0.82
		GL4AddQuad(px + sinA * noseFwd, pz + cosA * noseFwd, width * 0.3, height * 0.2, angle, noseR, noseG, noseB, color[4])

		-- Tail fins: wider short quad at the back, swept shape
		local finFwd = -height * 0.18
		GL4AddQuad(px + sinA * finFwd, pz + cosA * finFwd, width * 2.0, height * 0.18, angle, finR, finG, finB, color[4] * 0.85)

		-- Exhaust glow at the very back
		local exhFwd = -height * 0.38
		local exhR, exhG, exhB = mc[10], mc[11], mc[12]
		GL4AddQuad(px + sinA * exhFwd, pz + cosA * exhFwd, width * 0.5, height * 0.1, angle, exhR, exhG, exhB, color[4] * 0.6)
	elseif pDefID and cache.weaponIsBomb[pDefID] then
		...
		GL4AddQuad(px + sinA * noseFwd, pz + cosA * noseFwd, bombW * 0.33, bombH * 0.18, bombAngle, 0.73, 0.73, 0.71, 1.0)
