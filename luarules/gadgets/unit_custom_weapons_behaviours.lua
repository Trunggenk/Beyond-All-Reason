...
local function torpedoWaterPen(projectileID)
	...
	if targetType == targetedUnit and targetID then
		...
		if targetX and positionX then
			closeToTarget = distance3dSquared(positionX, positionY, positionZ, targetX, targetY, targetZ) < terminalCorrectionDistanceSq
		end
	...
	local terrainCorrectedY = velocityY - normalY * (velocityX * normalX + velocityY * normalY + velocityZ * normalZ)
